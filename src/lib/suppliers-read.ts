...
/**
 * Server-side read helpers for the Suppliers domain (post Prisma-decommission).
 *
 * Convex is the sole owner of supplier data — the Prisma `supplier` table (and
 * the FKs `asset`, `bulk_asset`, `project_line_item`, `sub_hire`, `supplier_order`,
 * and `supplier_model_rate` carried to it) was dropped at cutover. All reads,
 * including cross-domain joins deep inside warehouse / category / PDF pipelines,
 * go through Convex via this helper / the `use-suppliers` hooks. See FEATUREDOCS/54.
 *
