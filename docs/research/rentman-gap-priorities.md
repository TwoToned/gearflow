...
| #26 | **Crew charge rates** | Medium | Medium | Labour is revenue for a production company, but Flow only knows crew *cost* — labour margin is invisible and every labour line is hand-priced. Add per-role charge rate (org default + override), auto-price labour service lines from assigned crew/roles. |
