...
/**
 * Thin CRUD for DiscordAccountLink (Convex table "discordAccountLinks"). GENERATED — Phase 2.
 *
...
export const getById = query({
  ...
  handler: async (ctx, { id }) =>
    await ctx.db.query("discordAccountLinks").withIndex("by_cuid", (q) => q.eq("id", id)).unique(),
});
...
export const create = mutation({
  ...
  handler: async (ctx, args) => await ctx.db.insert("discordAccountLinks", args),
});
