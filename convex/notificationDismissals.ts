...
/**
 * Thin CRUD for NotificationDismissal (Convex table "notificationDismissals"). GENERATED — Phase 2.
 *
...
export const getById = query({
  ...
  handler: async (ctx, { id }) =>
    await ctx.db.query("notificationDismissals").withIndex("by_cuid", (q) => q.eq("id", id)).unique(),
});
...
export const create = mutation({
  ...
  handler: async (ctx, args) => await ctx.db.insert("notificationDismissals", args),
});
