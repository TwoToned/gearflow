/**
 * `/link [email]` — starts GearFlow account enrollment. Open to anyone in the
 * guild (the invoker is by definition not yet linked). Always ephemeral. The
 * service returns a CONSTANT result for unmatched emails, so this command
 * renders one fixed "if that email is on file…" message and never reveals
 * whether the email exists. The only non-constant branch is "already_linked"
 * (the caller's own account, safe to show).
 */
