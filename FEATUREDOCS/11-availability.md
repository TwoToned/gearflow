...
4. **Every `computeOverbookedStatus`/`reconstructOverbookedStatus`/`checkAvailability` caller must resolve `getProjectWindow`/`getProjectWindowDates` (`src/lib/project-window.ts`) first — never pass a project's raw `rentalStartDate`/`rentalEndDate`.** A project's gear-committed window (`projectStartDate`/`projectEndDate`, WS2 #941) can diverge from — or exist when — its rental window doesn't. Passing the raw rental pair silently drops into the "Dateless Stock Checks" branch above (only-this-project's-own-demand) whenever `rentalStartDate`/`rentalEndDate` are unset, even though a real committed window exists, which UNDERCOUNTS cross-project overlap and can hide a real overbooked badge — or hide the "this will overbook, confirm?" checkbox in the add/edit-line-item dialogs, since those also call `checkAvailability` with whatever window they're handed. Found as a live bug (2026-09) in five places: `getProject`/`getProjectIssueFlags` (`src/server/projects.ts`), `enrichProjectDetailOverbooked` (`src/lib/project-detail-reconstruct.ts`), the `overbooking.bundle` query args (`src/hooks/use-native-project-equipment.ts`), and the project detail page's `rentalStart`/`rentalEnd` fed straight into `EquipmentTabSlot` (`src/app/(app)/projects/[id]/page.tsx`) — all five read the raw fields instead of the resolved window. **Billing is the one deliberate exception** — `FinanceTabSlot` on that same page keeps reading `rentalStart`/`rentalEnd` raw, because pricing runs on the chargeable window, never the gear-committed one (see `project-dates.ts`). Regression test: `src/lib/project-detail-reconstruct.test.ts` ("uses the gear-committed project window, not raw rental dates…").
