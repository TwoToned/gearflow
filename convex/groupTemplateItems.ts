...
/**
 * Thin CRUD for GroupTemplateItem (Convex table "groupTemplateItems"). GENERATED — Phase 2.
 *
...
export const getById = query({
  ...
  handler: async (ctx, { id }) =>
    await ctx.db.query("groupTemplateItems").withIndex("by_cuid", (q) => q.eq("id", id)).unique(),
});
...
export const create = mutation({
  ...
  handler: async (ctx, args) => await ctx.db.insert("groupTemplateItems", args),
});
