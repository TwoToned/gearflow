...
describe("checkInItems — fulfillment model round-trip", () => {
  ...
  // NOTE: the former "completeCheckAndStore on a multi-unit line" case was removed
  // when completeCheckAndStore went browser-direct (convex/checkRecordWrites.ts). Its
  // return delegation to returnLineUnits is now exercised by checkRecordWrites.test.ts
  // (store returns via checkinItemsCore) and by the checkInItems cases above.
});
