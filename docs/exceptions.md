...
| R-8.8.3 | The `e2e-harness` CI job (critical-flows #2, #5-9: sign-in through the primary revenue path) was never verified green — the docker-in-CI networking path from the self-hosted Convex backend container to the app's JWKS endpoint (`host.docker.internal`) failed on the GitHub-hosted runner. It ran `continue-on-error: true` so it wasn't gating merges, but it consistently failed/timed out on every PR with no signal, adding noise and burning CI minutes. The job definition is **temporarily removed** from `.github/workflows/ci.yml` (specs and harness scripts are untouched) until the networking issue is root-caused and a run has gone green locally against the same setup. Critical flow #1 (login, `e2e` job) stays blocking and unaffected. | `e2e-harness` job only, in `.github/workflows/ci.yml` | Jayden (eng) | 2026-08-22 |
