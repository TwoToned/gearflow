...
- ~~`categories.ts`~~ **DONE (#524)** — deleted; getCategories/tree/caseIds dead (reactive useCategories); getCategory→`convex/categories.ts detail` composite + searchContainerAssets→`containerAssetSearch` + 3 writes→`categoriesWrites.ts` (delete-guards, dup-id). categories-read.ts KEPT. `locations.ts` (single-default toggle+delete-guard+cascade→mutation), `suppliers.ts` (delete-guard+rate cascade→mutation)
