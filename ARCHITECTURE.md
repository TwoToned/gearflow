...
| 59 | [Bulk Operations](./FEATUREDOCS/59-bulk-operations.md) | Multi-select + bulk actions on list surfaces. Phase 1: project **Equipment** line items — row checkboxes + select-all + shared `BulkActionBar`, batched delete/move/edit server actions (`removeLineItemsBatch`, `moveLineItemsToGroup`, `updateLineItemsBatch` — one recalc + one audit each). Scales to services/crew/tasks next. |
