...
v1 ships a working bot: `/link` enrollment, `/asset lookup`, `/project info|crew|doc`, `/asset fault → DamageEvent`, automatic per-project channels with crew permission sync, status-driven create/archive lifecycle. All Discord configuration (bot token, app id, guild id, categories, status rules, behavior toggles) lives in the `DiscordIntegration` row; there is no `.env` for Discord.

> **Update (v0.19.0.0): the bot is back in its own process.** v1 ran the bot in-process via `instrumentation.ts` (the standalone service was deleted in v0.12.0.0). That coupled bot stability to web stability: an unhandled discord.js gateway error crashed the whole Next.js server, surfacing as intermittent Cloudflare 502s with no app logs. The bot now runs as a separate pm2 app, `gearflow-discord-bot` (`scripts/discord-bot.ts`), still importing the same `@/lib` service layer (invariants run once), so a gateway crash only restarts the bot. The web app controls it through the DB instead of in-process calls (`botDesiredState`/`botRestartRequestedAt`/`lastHeartbeatAt`; `src/lib/discord/bot-control.ts` + `bot-supervisor.ts`). See `docs/operations/discord-bot.md`. The notes below that say "in-process" / "restartBot()" describe the v1 model and are superseded by this.
