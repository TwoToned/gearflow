...
| T-P4 Monthly cost budget (metered) | **Maps $15 · Resend $15 · Convex plan cap** *(confirmed 2026-07-22)* | Ceilings for the metered vendors. **Usage now tracked** (2026-07-23, #764): a `vendor_usage` PostHog event fires per billable Resend send (`src/lib/email.ts`) and per Maps Autocomplete/Place Details request (`src/components/ui/address-input.tsx`), a "Vendor usage (T-P4)" insight (owner Jayden Nawotka, monthly review) is to be created once live event volume confirms, same precedent as the CWV insights. **80%-alerting is NOT yet wired** — needs either a persistent monthly counter or a PostHog query-capable key (only the write-only ingestion key is configured); target lines worked out in `src/lib/vendor-cost-tracking.ts` (Resend 2,400 sends/mo, Maps $12/mo) for whichever lands first. The PostHog plan's 5-alert cap (see `docs/convex-observability-runbook.md`) is also already full. See FEATUREDOCS/61. |
