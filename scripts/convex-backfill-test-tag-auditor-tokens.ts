/**
 * One-time (re-runnable) backfill: copy the TestTagAuditorToken table from Prisma
 * into Convex.
 *
 ...
 *
 *   pnpm tsx --env-file=.env --env-file=.env.local scripts/convex-backfill-test-tag-auditor-tokens.ts
 *
...
type CreateArgs = FunctionArgs<typeof api.testTagAuditorTokens.create>;

async function main() {
  const convex = await getConvexClient();
  const rows = await prisma.testTagAuditorToken.findMany({ orderBy: { createdAt: "asc" } });
  console.log(`Found ${rows.length} test & tag auditor tokens in Prisma.`);

  let created = 0;
  let skipped = 0;
  for (const r of rows) {
    const __res = await convex.mutation(
      api.testTagAuditorTokens.createIfMissing,
