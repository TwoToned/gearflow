...
| **S3 bucket `gearflow-uploads`** | `.env*`, prod Garage box | Bucket holds **real uploaded files**. Renaming orphans every stored object; app 404s on existing media | **Keep the bucket name** (internal, invisible) OR do a full object copy + swap. Recommend KEEP. |
