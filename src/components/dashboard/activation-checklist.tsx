...
function buildItems(state: {
  ...
  return [
    {
      key: "model",
      label: "Add a piece of gear you own",
      cta: "Add a model",
      href: "/assets/models/new",
      done: state.hasModel,
      meta: state.hasModel ? (state.firstModelName ?? undefined) : undefined,
    },
    {
      key: "asset",
      label: "Add a real unit of it",
      cta: "Add an asset",
      href: state.firstModelId ? `/assets/registry/new?modelId=${state.firstModelId}` : "/assets/models/new",
      done: state.hasAssetOnFirstModel,
    },
    {
      key: "project",
      ...
      meta: state.hasProject ? (state.firstProjectName ?? undefined) : undefined,
    },
    {
      key: "lineItem",
      label: "Put that gear on the job",
      cta: "Add it to the job",
      href: state.firstProjectId ? `/projects/${state.firstProjectId}?tab=equipment` : "/projects/new",
      done: state.hasModelLineItemOnFirstProject,
    },
  ];
}
...
export function ActivationChecklist({ orgId }: { orgId: string | undefined }) {
  ...
  const activeKey = items.find((i) => !i.done)?.key;
