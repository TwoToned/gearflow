  ...
  "relative inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-full font-sans font-semibold text-[14px] transition-[transform,box-shadow,background-color,color] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--paper)] disabled:cursor-not-allowed disabled:opacity-45 disabled:shadow-none disabled:translate-y-0 disabled:hover:translate-y-0 [&_svg]:size-5 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        ...
        // legacy aliases retained so the existing app keeps compiling while surfaces are progressively redesigned.
        default: "bg-red text-white shadow-[0_3px_0_var(--red-700)] hover:-translate-y-px active:translate-y-[2px] active:shadow-[0_1px_0_var(--red-700)]",
        outline: "border-2 border-border bg-transparent text-ink hover:bg-ink hover:text-paper active:translate-y-px",
        secondary: "border-2 border-border bg-elev text-ink hover:bg-paper-2 active:translate-y-px",
        destructive: "bg-red text-white shadow-[0_3px_0_var(--red-700)] hover:-translate-y-px active:translate-y-[2px] active:shadow-[0_1px_0_var(--red-700)]",
        link: "bg-transparent p-0 text-ink underline-offset-4 hover:text-red hover:underline active:opacity-80",
      },
      size: {
        sm: "h-9 px-4",
        default: "h-11 px-5",
...
export interface ButtonProps
  ...
  asChild?: boolean;
  /** Compatibility with existing app surfaces that used the old shadcn `render={<Link />}` API. */
  render?: unknown;
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, render, loading = false, disabled, children, ...props }, ref) => {
    const classes = cn(buttonVariants({ variant, size, className }));
    if (React.isValidElement<{ className?: string; children?: React.ReactNode }>(render)) {
