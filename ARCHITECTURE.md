...
| 56 | [Agent-Accessible API + MCP](./FEATUREDOCS/56-api-mcp.md) | **Reinstatement underway — phases 0 + 1 landed 2026-07-28 (#996, #997).** The API mints a short-lived AGENT token (`sub` = the key's acting user, `akid` = the key, never `svc`) instead of the SERVICE token, so every in-mutation gate applies with no per-operation security code and the 602 `requireService`-gated functions are unreachable by construction. Built: agent auth kind, `requireAgentScope` (RBAC ∩ key scopes, re-read in-transaction), fail-closed resource-less reads, agent rate buckets + 50-item bulk cap, `apiIdempotency` ledger + deterministic ids, error envelope, and the offline contract registry with four CI gates. **No general request surface yet — that is Phase 2 (#998);** today's only route is the throwaway Phase-0 prover. Coverage table: [`docs/api-coverage.md`](./docs/api-coverage.md). |
