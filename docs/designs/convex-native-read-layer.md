...
- **Migrate the Postgres-only write paths flagged in §1/§3** (`org-members.ts`, `custom-roles.ts`, `org-calendar.ts`) into Convex here too — that retires the mirror's fail-closed complexity (once membership/roles are written natively in Convex, there's no Prisma→Convex mirror to keep in sync).
  - **⚠️ RECONCILIATION (2026-07-03): this bullet CONFLICTS with §7 and is NOT done — deliberately.** §7's do-not-touch list keeps **Better Auth + the `organization`/`member`/`invitation` Postgres tables** as the source of truth ("auth stays in Postgres"). `member`/`organization` are **Better-Auth org-plugin tables** — the org plugin (`src/lib/auth.ts`) reads/writes them for invites, role assignment, and membership. Making Convex their source of truth would break Better Auth's ownership of them, which §7 forbids. `custom-roles` is a custom table but its writes are meaningless split from `member` (which references `custom:<id>`), and `org-calendar` settings are custom columns **on the Better-Auth `organization` table**. So all three stay Postgres-source-of-truth + Convex-mirror, exactly as §3.4 specifies. **§7 (the explicit firmer constraint) wins over this aspirational Phase-5 line.** The mirror already gives the authoritative read side for RBAC (revocation fails closed to Convex first, §3.3.4), so the guard reads ARE effectively authoritative today — the "convert to authoritative" follow-up is moot without moving the source, which we won't. Moving org membership into Convex is really **Tier E** (auth into Convex), which is **out of scope**.

- **Audit-read migration (5c enabler) — NOT done, and correctly BLOCKED until the write flags flip.** 5c writes new audits to Convex `activityLogs` **only when a `NATIVE_*_WRITES` flag is on** (all OFF today). So Convex `activityLogs` is **empty in prod**; pointing the activity-log screens (`src/server/activity-log.ts`) at it now would show a blank history. The correct sequence: (1) flip the write flags live per domain, (2) backfill the historical Postgres `activityLog` → Convex `activityLogs` (reuse the `scripts/convex-backfill-*.ts` pattern), (3) THEN migrate the read screens to a reactive `activityLogs` useQuery + drop the transitional Postgres `logActivity` dual-writes. This is a real sequencing dependency, not a punt — the mutations + atomic-audit writer (`convex/lib/audit.ts`) that unblock it are already shipped.

- **5d optimistic client updates — representative shipped; full rollout is mechanical follow-up.** 5a–5c (RBAC + invariants + atomic audit in the mutation) are done for every write, which is what makes a browser/user token able to call the mutation safely. 5d (swap the server-action call for `useMutation(api.x).withOptimisticUpdate(...)`) is a per-write **client** change — done for the representative user-facing write; rolling it to the rest is mechanical (each: thread the current user as `actor`, generate `auditId`/`now` client-side, patch the surface's native `useQuery` in the optimistic updater).
