<!-- Owner: Jayden Nawotka · Last reviewed: 2026-08-02 (review quarterly — POLICY.md R-5.5) -->
...
| T-P10 Mira agent-loop bound | **6 tool-call iterations · 60s wall clock** | `src/lib/mira/agent-loop.ts`'s `runMiraAgentLoop` hard-stops a confused model rather than looping/burning tokens indefinitely — every dispatch()-level call still passes through T-P9's `agentRead`/`agentWrite` limits regardless. No platform cost line here: each org supplies its OWN LLM API key (`miraOrgSettings`, `/settings/mira` — OpenRouter by default, or any OpenAI-compatible endpoint via the optional `baseUrl`) and pays for its own Mira usage directly — the platform has no shared LLM credential and bears no spend, so no R-9.2 80%-alert applies to this budget (it bounds latency/runaway-loop risk, not cost). |
