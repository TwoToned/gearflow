...
//
// TodayWorkListWidget smoke coverage (D10C — moved here from the now-hidden
// `/today` page's own smoke test; the widget is the SAME implementation that
// used to be hosted there and is now hosted on the dashboard board instead,
// per its own header comment). Renders without a live Convex provider (every
// data hook is mocked at its module boundary), org-tz bucketing puts a task
// in the right section, a mention surfaces in Triage, the all-clear empty
// state renders when nothing is open, and the done checkbox calls the
// existing task-status mutation.
...
import { TodayWorkListWidget } from "../today-work-list-widget";

function renderWidget() {
  return render(<TodayWorkListWidget orgId="org1" />);
}
...
describe("TodayWorkListWidget (smoke)", () => {
  beforeEach(() => {
