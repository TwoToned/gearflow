...
| 56 | [Production Deployment](./FEATUREDOCS/56-deployment.md) | GitHub Actions builds the prod image (16GB runners — droplet OOMs on `next build`) → GHCR → Coolify pulls + runs on DigitalOcean. Runtime-entrypoint migrations, build-only secret placeholders, Convex prod auth bridge, GHCR registry creds, manual-migration runbook (container terminal), baked changelog (`build-info.json`), and Garage (S3) bootstrap. |
