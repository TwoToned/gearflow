...
> **Status: Phase A STARTED (2026-06-16).** Everything below Phase 0 ("Done") is
> shipped and stable. Phase A read-rewiring is underway — first leaf surface (the
> Test & Tag reports) converted; see the Phase A progress log below. Phases B/C
> not started. Tracked as tasks #4 (Phase A), #5 (Phase B), #6 (Phase C).
...
- **`test-tag-reports.ts` — DONE (2026-06-16, PR #1).** First leaf. Read-only
  reports/CSV/counts → Convex via `src/lib/test-tag-read.ts`. `testedBy` (User)
  stays Prisma. Validated with unit tests + a row-for-row golden-diff vs Prisma on
  seeded dev data. Details: FEATUREDOCS/54 "Phase A — read-rewiring". Also fixed
...
empty — `npm run seed && npm run seed:test-tag`, then
