...
| 70 | [Project Version Switcher](./FEATUREDOCS/70-project-version-switcher.md) | Phase 3 of #1080 (#1093) — `?v=` header switcher + read-only bar for viewing a past project version. One pure mapper (`projectSnapshotEntries`) turns a snapshot's (or the live state's, via `currentEntries` — identical shape) entries into Equipment/Labour/Finance DTOs; a dedicated read-only render surface (not an `aria-disabled` sweep of the live tabs — a deliberate, documented scope call) renders them behind `ProjectVersionContext`. Tasks/Files stay live and say so; Notes (the project's own text fields) IS captured and shows read-only. No promote action yet (Phase 2/#1089 unshipped) — look-only. |
