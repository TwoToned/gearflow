      ...
      <DialogContent className="max-w-2xl">
        ...
        <div className="flex gap-1.5">
          <Button
            ...
            size="sm"
            disabled={!previousQuote?.snapshotId}
            onClick={() => setMode("previous")}
            title={!previousQuote?.snapshotId ? "There's no earlier revision to compare against." : undefined}
          >
            vs previous
          </Button>
          <Button type="button" variant={mode === "current" ? "primary" : "line"} size="sm" onClick={() => setMode("current")}>
            vs the job now
          </Button>
        </div>
