...
export function PullSlipPDF({ org, project }: PullSlipPDFProps) {
  ...
  const equipmentItems = project.lineItems.filter(
    (i) => i.status !== "CANCELLED" && !i.isKitChild && !i.isPrepChild
  );
                    ...
                    <View style={idx % 2 === 0 ? s.tableRow : s.tableRowAlt}>
                      ...
                      <View style={{ flex: 3 }}>
                        <View style={{ flexDirection: "row", alignItems: "center", gap: 4 }}>
                          ...
                          {item.preppedIn && (
                            <Text style={{ fontSize: 6, color: "#059669", backgroundColor: "#d1fae5", paddingHorizontal: 3, paddingVertical: 1, borderRadius: 2, fontFamily: "Helvetica-Bold" }}>PREPPED: {item.preppedIn}</Text>
                          )}
                        </View>
                        {item.notes && (
