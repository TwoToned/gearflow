...
const EXCLUDE_PATTERNS = [
  ...
  /^src\/generated\//,
  /^convex\/_generated\//,
  // The `*.generated.ts` naming convention, for generated files that live BESIDE
  // the code that consumes them rather than in a generated/ directory — e.g.
  // `src/lib/api/registry.generated.ts`, which is ~50k lines of extracted Convex
  // contract. Reviewing it line by line is meaningless (it is regenerated by
  // `pnpm run api:registry` and a staleness gate fails CI if it drifts), so
  // counting it toward a review-effort budget measures the wrong thing.
  /\.generated\.ts$/,
  /(^|\/)__fixtures__\//,
  /(^|\/)fixtures\//,
];
