...
export default function MiraSettingsPage() {
  ...
  const form = useForm<MiraSettingsFormValues>({
    ...
    defaultValues: { openRouterApiKey: "", model: DEFAULT_MIRA_MODEL, writeAccessEnabled: false },
  });
  ...
  useEffect(() => {
    if (!settings) return;
    form.reset({ openRouterApiKey: "", model: settings.model || DEFAULT_MIRA_MODEL, writeAccessEnabled: settings.writeAccessEnabled });
    // eslint-disable-next-line react-hooks/exhaustive-deps -- re-seed only when the underlying doc changes
  }, [settings?.model, settings?.writeAccessEnabled]);
  ...
  const disconnectMutation = useServerMutation({
    ...
    onSuccess: () => {
      toast.success("Disconnected Mira's OpenRouter key");
      void refetch();
    },
    onError: (e) => toast.error(e.message),
  });
  ...
  return (
    <div className="space-y-8">
      <FormSection
        ...
        description="Give Mira real language-model reasoning: her own OpenRouter API key and model, brought by this org."
      >
        <SettingsCard>
          <div className="flex items-center gap-2">
            {hasApiKey ? (
              <Badge status="ok">
                <CheckCircle2 className="h-3.5 w-3.5" /> OpenRouter connected
              </Badge>
            ) : (
            ...
            >
              <div className="space-y-2">
                ...
                {form.formState.errors.openRouterApiKey && (
                  <p className="t-micro text-destructive">{form.formState.errors.openRouterApiKey.message}</p>
                )}
                <p className="t-micro text-fg-3">
                  ...
                  . This org&apos;s own key pays for its own Mira usage — nothing is shared across orgs.
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="t-micro text-fg-3">
                  Any OpenRouter model slug that supports tool calling, e.g. <code className="rounded bg-paper-2 px-1 py-0.5 t-micro">anthropic/claude-sonnet-4.5</code>,{" "}
                  <code className="rounded bg-paper-2 px-1 py-0.5 t-micro">openai/gpt-5</code>, or <code className="rounded bg-paper-2 px-1 py-0.5 t-micro">google/gemini-2.5-pro</code>.
                </p>
              </div>
              ...
              <div className="flex items-start justify-between gap-4 rounded-[var(--r)] border border-border p-3">
                <div className="space-y-1">
                  ...
                  <p className="t-micro text-fg-3">
                    Off: Mira can only read data and answer questions. On: Mira may also create/update projects, assets,
                    crew assignments, etc. — always as the asking member, never with more access than their own role
                    already grants, and every high-risk action (delete, financial void, warehouse dispatch, bulk changes)
                    still stops and asks a human to confirm before it runs. Flipping this re-provisions every member&apos;s
                    Mira key immediately.
                  </p>
                </div>
                <Controller
