...
| R-8.8.3 | The `e2e-harness` CI job (critical-flows #2-10: sign-in through the primary revenue path) has not yet been verified green on a GitHub-hosted runner. Root-caused (2026-07-23): NOT the docker-in-CI JWKS networking path as first suspected (already correctly mapped via `host.docker.internal:host-gateway`) — Playwright's `webServer` was spawning `next dev`, and the harness's much larger flow set (vs. the `e2e` job's plain `/login` smoke) triggers enough Turbopack on-demand compilation under CI load to hit a known Next dev-server crash (`uncaughtException`/`ECONNRESET` on a mid-compile request abort). Fixed by running a prebuilt `next start` for this job only (`E2E_PROD_SERVER=1`, `playwright.config.ts`) — proven passing locally against the harness, not yet proven on a GitHub-hosted runner. The job runs `continue-on-error: true` until a run (ideally a few in a row) is confirmed green there. Critical flow #1 (login, `e2e` job) stays blocking and unaffected throughout. | `e2e-harness` job only, in `.github/workflows/ci.yml` | Jayden (eng) | 2026-08-22 |
