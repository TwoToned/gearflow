...
| 62 | [Incident Reporting](./FEATUREDOCS/62-incident-reporting.md) | "Report Issue" — the in-app successor to the deleted Discord `/fault` command (#898). Reuses `MaintenanceRecord` (type=REPAIR, `incidentType`/`incidentSeverity` set) rather than a parallel model; flips the reported asset's status immediately (IN_MAINTENANCE / LOST). Available on CHECKED_OUT project lines, the warehouse deploy/return tabs, and the asset detail page. A check-item FAIL now also opens an immediate REPAIR record (alongside the existing 2-of-3-fails predictive trigger, unchanged). |
